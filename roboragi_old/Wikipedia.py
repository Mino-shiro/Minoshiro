...
wiki.headers.update({
                        'User-Agent': 'Roboragi - An Anime/Manga Reddit Bot - Contact /u/Nihilate on Reddit'})
...
def getThingURL(searchText, searchType=None):
    try:
        request = wiki.get(
            BASE_API_URL + 'action=query&format=json&list=search&utf8=1&srsearch=' + searchText,
            timeout=10)
    except:
        return None
    ...
    closestThings = difflib.get_close_matches(searchText.lower(),
