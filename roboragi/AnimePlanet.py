...
async def getAnimeURL(searchText):
    try:
        ...
        async with session.get(
                                BASE_URL + "/anime/all?name=" + searchText.replace(
                        " ", "%20"), timeout=10) as resp:
            ...
            if ap.find('.cardDeck.pure-g.cd-narrow[data-type="anime"]'):
                ...
                closestName = difflib.get_close_matches(searchText.lower(),
                                                        [x['title'].lower() for
                                                         x in animeList], 1,
                                                        0.85)[0]
                closestURL = ''
    ...
    try:
        if authorName:
            async with sessions.get(
                                                    BASE_URL + "/manga/all?name=" + searchText.replace(
                                            " ",
                                            "%20") + '&author=' + authorName.replace(
                            " ", "%20"), timeout=10) as resp:
                html = await resp.text()
            if "No results found" in html:
                ...
                async with session.get(
                                                        BASE_URL + "/manga/all?name=" + searchText.replace(
                                                " ",
                                                "%20") + '&author=' + rearrangedName.replace(
                                " ", "%20"), timeout=10) as resp:
                    html = await resp.text()
